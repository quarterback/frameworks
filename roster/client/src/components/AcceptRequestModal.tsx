...
export default function AcceptRequestModal({ request, onClose, onAccepted }: Props) {
  const { credits } = useAuth();
  const [selectedCredits, setSelectedCredits] = useState(Math.min(request.creditsSent, 3));
  ...
  const handleAccept = async () => {
    ...
    try {
      await api.acceptRequest(request.id, selectedCredits);
      onAccepted();
    } catch (err) {
      ...
      <div className="relative bg-primary rounded-xl shadow-xl max-w-md w-full p-6 space-y-6">
        ...
        <div className="bg-secondary rounded-lg p-4">
          <p className="text-sm text-text/70">
            <span className="font-medium text-text">{request.fromProfile?.name}</span> sent you{' '}
            <span className="font-medium text-accent">{request.creditsSent} credit{request.creditsSent > 1 ? 's' : ''}</span>
          </p>
          {request.message && (
            <p className="mt-2 text-sm text-text/60 italic">"{request.message}"</p>
          )}
        </div>
        ...
        <div className="space-y-4">
          <label className="block text-sm font-medium text-text/80">
            How many credits will you send back?
          </label>
        ...
        <div className="flex space-x-3">
          ...
          >
            {loading ? 'Accepting...' : `Accept (${selectedCredits})`}
          </button>
        </div>
      </div>
    </div>
